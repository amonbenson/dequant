...
class PlayCommand:
    ...
    sample: Annotated[int | None, tyro.conf.arg(prefix_name=False)] = 0
...
@dataclass
class DequantizeRtCommand:
    """Use a trained model to dequantize an input stream of HOV steps"""
...
class Args:
    ...
    command: (
        ...
        | Annotated[DequantizeRtCommand, tyro.conf.subcommand("dequantize-rt", prefix_name=False)]
...
def main():
    ...
    match args.command:
        ...
        case DequantizeRtCommand():
            cli.run_dequantize_rt(
                args.command.checkpoint,
            )
        case AppCommand():
            cli.run_app()
