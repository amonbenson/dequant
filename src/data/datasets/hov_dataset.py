...
class HOVDatasetConfig:
    ...
    glob_pattern: str = "*.npz"  # filter which npz files are loaded (e.g. "egmd*.npz" or "lmd*.npz")
...
class HOVDataset(Dataset):
    ...
    def _build_index(self):
        ...
        npz_files = sorted(Path(self.config.dir).glob(self.config.glob_pattern))
        if not npz_files:
            raise FileNotFoundError(f"No files matching '{self.config.glob_pattern}' found in {self.config.dir}. Did you run preprocess?")
